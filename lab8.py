...
class Contract:
    def __init__(self, sid: str, htype: str, manager: str, amount: float):
        self.sid = sid
        # три атрибута: housing_type, manager, amount
        self.htype = htype
...
class ContractsApp:
    def __init__(self, root):
        self.root = root
        self.root.title("ЛР Вариант 15 — Договоры на продажу жилья")
        self.contracts = []
        self._setup_style()
        self.build_gui()
    ...
    def build_gui(self):
        ...
        ttk.Button(top, text="Диаграмма по видам жилья", style="Calm.TButton", command=self.show_pie_by_type).grid(row=0, column=2, padx=6)
        ttk.Button(top, text="Диаграмма по менеджерам", style="Calm.TButton", command=self.show_pie_by_manager).grid(row=0, column=3, padx=6)
    ...
    def parse_csv_line(self, line):
        """Парсит строку CSV с учетом кавычек и запятых внутри полей"""
        line = line.strip()
        if not line:
            return []
        ...
        try:
            with open(path, encoding='utf-8') as f:
                ...
                for nr, line in enumerate(f, start=1):
                    ...
                    # Парсим строку с помощью нашего метода
    ...
    def show_pie(self, counts: dict, title: str):
        ...
        for i, (label, cnt) in enumerate(items):
            ...
            self.canvas.tag_bind(arc, '<Button-1>',
                lambda e, l=label, c=cnt: messagebox.showinfo('Сегмент', f"{l}: {c} ({c/total*100:.1f}%)"))
            start += extent
        lx, ly = cx + r + 30, cy - r
        for i, (label, cnt) in enumerate(items):
            y = ly + i*26
